...
* Acording to the above ACF and PACF plots (Plot Series 1) the series in non-stationary. The ACF decays very slowly over time, indicating the presence of deterministic or stochastic trend.

## c) Is the CPI series stationary? Formally test the stationarity of the series.

* When we preform the Augmented Dicky-Fuller test on the original data we obtain a p-value of `r adf1$p.value ` which indicates we FAIL to REJECT the null and determine the series is nonstationary.

* When we take the first difference of the series, (Plot Series 2) we see some indications that ther series has become stationary, but it is still possible to see a trend in the data. When we formally test the differenced data with the ADF test we obtain a p-value of `r adf2$p.value ` which indicates the series is stationary.

* Just out of curiosity I take the second difference of the CPI data and plot the series (Plot Series 3). This time series now apeears to be completely stationary. It is interesting to see the non-constant variance though; which will be addressed in problem 2. When testing with ADF we obtain a p-value of `r adf3$p.value `. Since the p-value indicates stationarity and the plot of the second differenced data look better; I will assume the data is stationary with two differences.
...
When we consider the AIC criterion we conclude that model 2 (ARIMA(0, 2, 1)) has the lowerest AIC and thus is the best model out of the three.
...
* It is not easy to assertain which model does a better job at forcating based on the above plots. There is a structural break in the data which is not modeled well by any of the models. However, aside from the structural break issue, the remainder of the data is resonably modeled by the forcast up to about four or five quarters. Beyond about five quarters the forcasting ability of the ARIMA(0,2,1) starts to fall apart.
...
* When we estimate the ARIMA(0,2,1)-ARCH(4) model a few things stick out. First, the paramater on the MA(1) portion of the ARMIA(0,2,1) is significant. Second, the Alpha1 and Alpha2 paramater are significant, however, the Alpha3 and Alpha4 paramaters are not. This would indicate that a ARCH(1) or ARCH(2) may be a better model than the ARCH(4). I would say, overall, the ARCH(4) model is not the appropriate model to implement for this time series.
...
* The ARIMA(0,2,1)-GARCH(1,1) seems to preform better than the other model variants. In the previous model ARMIA(0,2,1)-ARCH(4), when we examined the QQ-Plot it appeared the errors were not following a nomal distribution. In the ARIMA(0,2,1)-GARCH(1,1) we used a student t-innovation for the conditional distribution. By viewing the QQ-Plot we see that using the student t-innovation has helped with the normality of the errors but according to the Jarque-Bera Test statistic the models errors are still not strictly following a normal distridution. The papramaters on the MA(1), Alpha1 and Beta1 are significant and the Q(10) and Q(20) most all point to a good model fit. Overall, I would say the ARIMA(0,2,1)-GARCH(1,1) is a good model for this time series.
...
y <- matrix(c(5, 1, 1, 1, 0, 3, 2, 3, 4, 1), nrow=10)
