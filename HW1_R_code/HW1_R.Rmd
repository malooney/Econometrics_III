...
i <- 1 # counter
temp <- matrix(nrow = reps, ncol = 8) # matrix to stuff results into dim= "reps" X k+3
x1 <- data$X2
x2 <- data$X3
...
temp[i, 5] <- covb[2]
temp[i, 6] <- covb[3]
temp[i, 7] <- unlist(bp_test[1])
temp[i, 8] <- gvHet_test$GlobalTest$DirectionalStat4$Value

i <- i+1
}
  
dimnames(temp) <- list(c(), c("beta_0", "beta_1", "beta_2", "covb0", "covb1", "covb2", "BP_testStat", "GV_HET_Test"))
